---
name: plan-trip
description: 当用户需要规划出行行程时使用此技能。触发词："规划行程"、"安排路线"、"我要去XX"、"从XX到XX"，或提供了日期、目的地等出行信息。依次调用 IntentionAgent、EventCollectionAgent、ItineraryPlanningAgent，三个 Agent 均接受 model 参数且 reply() 为异步。
---
